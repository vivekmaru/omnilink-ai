    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="bg-black/[0.02] dark:bg-white/[0.02] border-b border-black/10 dark:border-white/10 font-mono font-semibold uppercase tracking-wider text-[10px] text-slate-400 dark:text-slate-500">
            <tr>
              <th className="p-3.5 w-10 text-center">
                <input
                  ...
                  className="rounded border-black/20 dark:border-white/20 text-[#d97757] focus:ring-0 cursor-pointer accent-[#d97757]"
                />
              </th>
            ...
            {links.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3.5 max-w-md">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="space-y-1 min-w-0">
                        <div className="font-newsreader text-base font-medium text-slate-900 dark:text-[#f7f6f3] group-hover:text-[#d97757] dark:group-hover:text-[#e08264] transition-colors truncate">
                          {link.title || link.url}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 pt-0.5">
                          {(link.isRssFeedItem || link.feedTitle) && (
                            <span className="inline-flex items-center gap-0.5 text-[9px] text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1.5 py-0.2 rounded font-medium">
                          ...
                          {((link.summary?.codeSnippets && link.summary.codeSnippets.length > 0) ||
                            ...
                            <span className="inline-flex items-center gap-0.5 text-[9px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1.5 py-0.2 rounded">
                              <Code2 className="w-2.5 h-2.5" /> code
                            </span>
                          )}
                          {(link.summary?.quotes?.[0] || link.summary?.quote || link.aiSummary?.quote) && (
                            <span className="inline-flex items-center gap-0.5 text-[9px] text-sky-600 dark:text-sky-400 bg-sky-500/10 px-1.5 py-0.2 rounded">
                          ...
                          {((link.summary?.keyTakeaways && link.summary.keyTakeaways.length > 0) ||
                            ...
                            <span className="inline-flex items-center gap-0.5 text-[9px] text-[#d97757] dark:text-[#e08264] bg-[#d97757]/10 px-1.5 py-0.2 rounded">
                  ...
                  <td className="p-3.5">
                    {link.category ? (
                      <span className="font-mono text-[11px] px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/5 text-slate-600 dark:text-slate-400">
                        {link.category}
                      </span>
                    ) : (
                  ...
                  <td className="p-3.5">
                    <div className="flex flex-wrap gap-1">
                      {link.tags?.slice(0, 2).map((t) => (
                        <span
                          key={t}
                          className="font-mono text-[10px] px-1.5 py-0.2 rounded bg-black/5 dark:bg-white/[0.04] text-slate-500"
                        >
                          #{t}
                        </span>
                      ))}
